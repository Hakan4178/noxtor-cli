...
static uint8_t g_db_key[NOX_KEY_LEN];
...
{
    if (crypto_generichash(hash_out, 32,
                           (const uint8_t *)onion, strlen(onion),
                           g_db_key, sizeof(g_db_key)) != 0) {
                                 ...
                                 uint8_t *cipher_out, size_t cipher_len)
{
    ...
    if (crypto_secretbox_easy(cipher_out, plain, plain_len, nonce, g_db_key) != 0) {
                                 ...
                                 size_t *plain_len_out)
{
    ...
    if (crypto_secretbox_open_easy(plain_out, cipher, cipher_len, nonce, g_db_key) != 0) {
        NOX_WARN(LOG_MOD_DB, "Kayıt MAC doğrulaması başarısız — hatalı anahtar veya bozuk veri");
        return NOX_ERR_AUTH;
    }
...
{
    if (!config_dir || !db_key) return NOX_ERR_CONFIG;
    if (g_db_ready) return NOX_OK;

    /* db_key'i statik alana kopyala */
    memcpy(g_db_key, db_key, NOX_KEY_LEN);

    char db_path[NOX_PATH_MAX];
    int n = snprintf(db_path, sizeof(db_path), "%s/contacts.db", config_dir);
    if (n < 0 || (size_t)n >= sizeof(db_path)) {
        explicit_bzero(g_db_key, sizeof(g_db_key));
...
{
    if (!g_db_ready) return NOX_ERR_STATE;
    if (!onion || !name || !noise_key) return NOX_ERR_DB;
    ...
    nox_err_t err = encrypt_payload((const uint8_t *)&cp, sizeof(cp),
                                    nonce, cipher, sizeof(cipher));
    explicit_bzero(&cp, sizeof(cp));
    if (err != NOX_OK) return err;
    ...
    const char *sql = "INSERT OR REPLACE INTO contacts (onion_hash, nonce, encrypted_payload) VALUES (?, ?, ?);";
...
{
    if (!g_db_ready) return NOX_ERR_STATE;
    if (!onion || !name_out || !noise_key_out || name_len == 0) return NOX_ERR_DB;
    ...
    err = decrypt_payload((const uint8_t *)cipher_ptr, (size_t)cipher_bytes,
                          ...
                          (uint8_t *)&cp, sizeof(cp), &plain_len_out);
    sqlite3_finalize(stmt);

    if (err != NOX_OK) return err;
    if (plain_len_out != sizeof(cp)) return NOX_ERR_DB;
...
{
    if (!g_db_ready) return NOX_ERR_STATE;
    if (!recipient_onion || !text) return NOX_ERR_DB;
    ...
    nox_err_t err = encrypt_payload((const uint8_t *)text, text_len,
                                    nonce, cipher, text_len + crypto_secretbox_MACBYTES);
    if (err != NOX_OK) return err;
    ...
    sqlite3_bind_blob(stmt, 1, hash, sizeof(hash), SQLITE_TRANSIENT);
    sqlite3_bind_blob(stmt, 2, nonce, sizeof(nonce), SQLITE_TRANSIENT);
    sqlite3_bind_blob(stmt, 3, cipher, (int)(text_len + crypto_secretbox_MACBYTES), SQLITE_TRANSIENT);
...
{
    if (!g_db_ready) return NOX_ERR_STATE;
    if (!recipient_onion || !send_fn) return NOX_ERR_DB;
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        err = decrypt_payload((const uint8_t *)cipher_ptr, (size_t)cipher_bytes,
                              (const uint8_t *)nonce_ptr,
                              (uint8_t *)plain, sizeof(plain), &plain_len_out);
        if (err != NOX_OK) {
