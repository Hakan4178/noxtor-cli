...
nox_err_t db_init(const char *config_dir, const uint8_t db_key[NOX_KEY_LEN]) {
  ...
  if (rc != SQLITE_OK) {
    NOX_ERROR(LOG_MOD_DB, "Veritabanı açılamadı: %s", sqlite3_errmsg(g_state.db));
    if (g_state.db) {
...
nox_err_t db_add_contact(const char *onion, const char *name, const uint8_t noise_key[NOX_KEY_LEN],
  ...
  if (rc != SQLITE_DONE) {
    NOX_ERROR(LOG_MOD_DB, "Kontakt ekleme hatası: %s", sqlite3_errmsg(g_state.db));
    err = NOX_ERR_DB;
    goto cleanup;
  }
...
nox_err_t db_list_contacts_with_summary(db_contact_summary_visitor_fn visitor, void *ctx) {
  ...
  if (rc != SQLITE_OK) {
    sqlite3_finalize(stmt);
    NOX_ERROR(LOG_MOD_DB, "Message query hazırlanamadı: %s", sqlite3_errmsg(g_state.db));
    DB_UNLOCK(); return NOX_ERR_DB;
  }
